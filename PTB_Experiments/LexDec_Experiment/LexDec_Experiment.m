function LexDec_Experiment(subject, stim_list, practice_list, num_blocks, is_in_MEG, run_practice, ...
    ...
    use_sound_response, sound_trigger_volume, sound_response_time, use_prime, prime_time, prime_ISI, is_debugging)
...
%   - use_prime: Set to 1 to use a prime word before the target word.
...
% about it.
block_stims = blockStims(stim_list, num_blocks, randomize_experiment_stims, use_prime);
practice_stims = blockStims(practice_list,1,randomize_practice_stims, use_prime);
...
function block_stims = blockStims(stim_list, num_blocks, randomize, use_prime)
...
while 1
    ...
    if use_prime
        [uniqueid condition item_num prime target f_trig p_trig t_trig] = ...
            strread(line,'%f%s%f%s%s%f%f%f','delimiter','\t');
    else
        [uniqueid condition item_num target f_trig t_trig] = ...
            strread(line,'%f%s%f%s%f%f','delimiter','\t');
        prime = {''};
        p_trig = -1;
    end
