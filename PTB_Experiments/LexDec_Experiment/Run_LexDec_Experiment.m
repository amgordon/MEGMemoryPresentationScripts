...
%   these variables.
% NOTE: Change these to 'Stim_list_Prime.txt' and 'Practice_List_Prime.txt'
%   if you're using primes before the word.
...
% NOTE: To use a prime, set the below parameters appropriately, and you
%   must fill out the _Prime.txt stim lists. 
%   They have an additional word in the stim files
%   before the target word and an additional trigger before
%   the target trigger (see txt samples - the first word is the prime
%   and the middle number is the prime trigger).


% Set to 1 to use a prime before the target word.
use_prime = 1;
...
LexDec_Experiment(subject_name, stim_list, practice_list, num_blocks, is_in_MEG, run_practice, ...
    ...
    use_sound_response, sound_trigger_volume, sound_response_time, use_prime, prime_time, prime_ISI)
